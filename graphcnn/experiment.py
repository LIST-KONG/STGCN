...
class GraphCNNExperiment(object):
    ...
    # 输出：准确率均值、均方差
    def run_kfold_experiments(self, no_folds=10, random_state=125):
        acc = []
        sensitivity = []
        specificity = []
        attr_arr = []
        ...
        for i in range(no_folds):
            ...
            cur_max, sensitivity_max, specificity_max, max_it, attr = self.run()
        ...
        with open('./results/%s.txt' % self.data_set_name, 'a+') as file:
            ...
            return mean_acc, std_acc, mean_sensitivity, std_sensitivity, mean_specificity, std_specificity, attr_arr

    # preprocess_data 函数
    def preprocess_data(self, data_set):
        self.graph_vertices = data_set[0].astype(np.float32)
        self.graph_adjacency = data_set[1].astype(np.float32)
        self.graph_labels = data_set[2].astype(np.int64)
        ...
        with tf.device("/cpu:0"):
            with tf.variable_scope('input') as scope:
                ...
                with tf.variable_scope('train_data') as scope:
                    ...
                    training_samples = [self.graph_vertices, self.graph_adjacency, self.graph_labels]
                ...
                with tf.variable_scope('test_data') as scope:
                    ...
                    test_samples = [self.graph_vertices, self.graph_adjacency, self.graph_labels]
    ...
    def create_loss_function(self):
        with tf.variable_scope('loss') as scope:
            ...
            cross_entropy = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.net.current_V, labels=self.net.labels))
            ...
            tp = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(self.net.labels, ones_like),
                                                      tf.equal(tf.argmax(self.net.current_V, 1), ones_like)),
                                       tf.float32))

            tn = tf.reduce_sum(tf.cast(tf.logical_and(tf.equal(self.net.labels, zeros_like),
                                                      tf.equal(tf.argmax(self.net.current_V, 1), zeros_like)),
                                       tf.float32))
            ...
            # self.reports['current_V'] = self.net.current_V
            self.reports['label'] = tf.argmax(self.net.current_V, 1)
    ...
    def run(self):
        ...
        if i < self.num_iterations:
            ...
            with tf.Session() as sess:
                ...
                # # 打印节点的attention值
                att_v = [i for i in tf.trainable_variables() if
                         i.name.startswith("gclstm/GC-LSTM/GCNLSTMCell/attention")]
                # print(sess.run(att_v[0]))
                ...
                return sess.run(
                    [self.max_acc_test, self.max_sensitivity_test, self.max_specificity_test, self.net.global_step,
                     self.net.attr], feed_dict={self.net.is_training: 0})
        else:
